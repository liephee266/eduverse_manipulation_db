...
Ce rapport détaille les différences structurelles entre deux versions d'un schéma de base de données PostgreSQL, fournies dans les fichiers `schema.sql` et `structure_uuid.sql`. L'analyse révèle que la différence fondamentale réside dans la **migration du type de données des clés primaires et étrangères de `integer` vers `UUID` (Universally Unique Identifier)**.

## 1. Différence Structurelle Majeure : Migration vers UUID

La version `structure_uuid.sql` a été mise à jour pour utiliser des UUIDs comme identifiants pour de nombreuses colonnes qui utilisaient des entiers (`integer`) dans la version `schema.sql`.
...
| Fonction | Arguments dans `schema.sql` | Arguments dans `structure_uuid.sql` |
| :--- | :--- | :--- |
...
| Colonne | Type dans `schema.sql` | Type dans `structure_uuid.sql` |
| :--- | :--- | :--- |
...
- **`schema.sql`** : La vue utilise la table `marking_periods` (qui est une vue dans `structure_uuid.sql` et une table dans `schema.sql` - *Note: une analyse plus approfondie a montré que `marking_periods` est une vue dans les deux cas, mais la source de la vue change*). La vue dans `schema.sql` fait référence à la table `school_marking_periods` et à la table `history_marking_periods`.
...
| Élément | `schema.sql` | `structure_uuid.sql` | Note |
| :--- | :--- | :--- | :--- |
...
| `bordereaux_details` | Présente (Table) | Absente (Table) | La table `bordereaux_details` est présente dans `schema.sql` mais absente dans la liste des `CREATE TABLE` de `structure_uuid.sql`. |
| `wx_rules_school` | Présente (Table) | Présente (Table) | Présente dans les deux. |
| `wx_reduction_eleve` | Présente (Table) | Absente (Table) | La table `wx_reduction_eleve` est présente dans `schema.sql` mais absente dans la liste des `CREATE TABLE` de `structure_uuid.sql`. |
| `wx_reduction_members` | Présente (Table) | Absente (Table) | La table `wx_reduction_members` est présente dans `schema.sql` mais absente dans la liste des `CREATE TABLE` de `structure_uuid.sql`. |

### 2.3. Autres Différences Mineures

- **Propriétaire des Objets** : Dans `schema.sql`, le propriétaire des objets est `postgres`. Dans `structure_uuid.sql`, le propriétaire est `wxu_school`.
- **Version de PostgreSQL** : `schema.sql` a été dumpé de la version 17.6, tandis que `structure_uuid.sql` a été dumpé de la version 15.8.
...
[1] `schema.sql` : Fichier de schéma de base de données original.
...
[4] `schema_structure.txt` : Liste des définitions de haut niveau extraites de `schema.sql`.
[5] `structure_uuid_structure.txt` : Liste des définitions de haut niveau extraites de `structure_uuid.sql`.
[6] `school_marking_periods_schema.txt` : Définition de la table `school_marking_periods` dans `schema.sql`.
